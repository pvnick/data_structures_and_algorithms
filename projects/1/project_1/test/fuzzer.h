...
#define FUZZ_OP_START operations.push_back({__FILE__, __LINE__, [&l](rv_func rv, eq_func eq, T& ret_item, rand_num_func rand_int) -> int {
...
namespace cop3530 {
    ...
    private:
        using rv_func = T(*)();
        using eq_func = bool(*)(const T&, const T&);
        using rand_num_func = int(*)(int);
        using op_func = std::function<int(rv_func, eq_func, T&, rand_num_func)>;
        size_t op_counter = 0;
        struct Op {
        ...
        /*
            we hold operations in a linked list. any of the ones implemented in this project should work.
        ...
        */
        ...
        OpResult do_operation(L& l, int rand_seed, std::string const& list_class_name) {
            ...
            FUZZ_OP_END
            FUZZ_OP_START
                ret_item = l.replace(rv(), rand_int(10));
                return l.size();
            FUZZ_OP_END
            FUZZ_OP_START
                return l.contains(rv(), eq);
            FUZZ_OP_END
            FUZZ_OP_START
                return l.is_empty();
            FUZZ_OP_END
            ...
            try {
                ...
                op_result.op_ret_val = op.go(rand_val, equals_func, op_result.returned_item, randi);
                ...
                op_result.error_message = "unhandled exception";
            }
            return op_result;
        }
        ...
        bool validate_lists(std::initializer_list<OpResult> op_results) {
            ...
            for (; op_results_iter != op_results.end(); ++op_results_iter) {
                ...
                if (result.op_ret_val != previous_result.op_ret_val) {
                    ...
                    throw;
                } else if ( ! equals_func(result.returned_item, previous_result.returned_item)) {
    ...
    public:
        ListFuzzer() = delete;
        ListFuzzer(unsigned int rand_seed, T rand_val_func(), bool equals(const T&, const T&)): sdal_inst(2) {
            srand(rand_seed);
            rand_val = rand_val_func;
            equals_func = equals;
        }
