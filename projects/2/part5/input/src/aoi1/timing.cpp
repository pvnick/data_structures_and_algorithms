...
inline size_t blank_hash_placeholder(std::string const& str) {
...
int main (int argc, char **argv)
{
    program_options::option_desc opt_descs[] = {
        {{"hash-func", required_argument}, true, "INT", "", "Which hash function to use (-1 means skip the hash and just extract the loop overhead)"},
        {{"hash-repititions", required_argument}, false, "ULONG", "1000000", "Number of times to hash each key (higher number = more accurate output, but takes longer)"},
        {{0, 0, 0, 0}}
    };
    ...
    opts.get_opt_val("hash-func", &hash_function_id);
    ...
    */
    ...
    case -1:
        //blank hash placeholder
        hash_function = &blank_hash_placeholder;
        break;
    case 0:
    ...
    utime /= hash_repititions;
    stime /= hash_repititions;
    printf("%.10f %.10f\n", utime, stime);
    return 0;
}
