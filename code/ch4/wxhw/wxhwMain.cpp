...
BEGIN_EVENT_TABLE(wxhwDialog, wxDialog)
    EVT_CLOSE(wxhwDialog::OnClose)
    EVT_BUTTON(idBtnQuit, wxhwDialog::OnQuit)
    EVT_BUTTON(idBtnAbout, wxhwDialog::OnAbout)
END_EVENT_TABLE()

wxhwDialog::wxhwDialog(wxDialog *dlg, const wxString &title)
...
wxhwDialog::~wxhwDialog()
{
}

void wxhwDialog::OnClose(wxCloseEvent &event)
{
    Destroy();
}

void wxhwDialog::OnQuit(wxCommandEvent &event)
{
    Destroy();
}

void wxhwDialog::OnAbout(wxCommandEvent &event)
{
